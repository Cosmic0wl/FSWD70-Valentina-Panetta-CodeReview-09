 ...
 -- Vehicles available on 30.7.2019 at agency branch nr 1 --
...
-- Closest workshop for branch nr 2 --

SELECT branch.name, workshop.name
FROM workshops workshop JOIN closest_workshops on fk_workshop_id = workshop.workshop_id JOIN agency_branches branch on fk_branch_id = branch.branch_id
WHERE fk_branch_id = 2;

-- Employee wage costs per branch --

SELECT branch.name, SUM(salary) 
FROM roles JOIN employees on fk_employee_id = employee_id JOIN agency_branches branch on fk_branch_id = branch.branch_id
GROUP BY branch_id;
